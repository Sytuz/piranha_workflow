...
namespace Piranha.Manager.Models
{
    ...
    public class PageEditViewModel : PageModel
    {
        ...
        public PageEditViewModel(Piranha.Manager.Services.WorkflowService workflowService, IChangeRequestService changeRequestService, IWorkflowStageService stageService, IUserResolutionService userResolutionService)
        {
        ...
        public async Task OnGetAsync()
        {
            ...
            // Load transition history for change requests related to this page
            try
            {
                // Get the page ID from route data
                if (RouteData.Values.TryGetValue("id", out var pageIdValue) && Guid.TryParse(pageIdValue?.ToString(), out var pageId))
                {
                    // Get change requests for this page content
                    var changeRequests = await _changeRequestService.GetByContentIdAsync(pageId);
